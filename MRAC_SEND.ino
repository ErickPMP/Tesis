...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
  ...
  incomingOut_IT1 = incomingReadings.Out_IT1;
  incomingOut_IT2 = incomingReadings.Out_IT2;
...
void loop() {
  ...
  Out_IT1_ML.number = incomingOut_IT1;
  Out_IT2_ML.number = incomingOut_IT2;
  ...
  //ATOMIC(){
    if (millis()-timeold >= 100){
      ...
      for(int i=0; i<4; i++){Serial.write(Out_IT1_ML.bytes[i]);}
      for(int i=0; i<4; i++){Serial.write(Out_IT2_ML.bytes[i]);}
...
/*
  ...
  */
  ...
  if(value_temporal != value){
    send_Data.ACTIVADOR = value;
    esp_err_t result = esp_now_send(broadcastAddress, (uint8_t *) &send_Data, sizeof(send_Data));
    if (result == ESP_OK) {Serial.println("Sent with success");}
    else {Serial.println("Error sending the data");}
  }
  value_temporal = value;
